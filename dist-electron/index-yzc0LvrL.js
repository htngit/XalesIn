"use strict";
const main = require("./main-BcFM4gE2.js");
const fs = require("fs");
const require$$0 = require("buffer");
const index$2 = require("./index-DTTCHfZv.js");
...
function requireGetStream() {
  ...
  const pump = index$2.requirePump();
...
function requireFdSlicer() {
  ...
  FdSlicer.prototype.write = function(buffer, offset, length, position, callback) {
    var self = this;
    self.pend.go(function(cb) {
      fs$1.write(self.fd, buffer, offset, length, position, function(err, written, buffer2) {
        cb();
  ...
  WriteStream.prototype._write = function(buffer, encoding, callback) {
    ...
    self.context.pend.go(function(cb) {
      if (self.destroyed) return cb();
      fs$1.write(self.context.fd, buffer, 0, buffer.length, self.pos, function(err2, bytes) {
        if (err2) {
          self.destroy();
          cb();
          callback(err2);
...
function requireExtractZip() {
  ...
  const debug = main.requireSrc()("extract-zip");
  ...
  const openZip = promisify(yauzl2.open);
...
const index = /* @__PURE__ */ main.getDefaultExportFromCjs(extractZipExports);

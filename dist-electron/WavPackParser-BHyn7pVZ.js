...
const BlockHeaderToken = {
  len: 32,
  get: (buf, off) => {
    const flags = index.UINT32_LE.get(buf, off + 24);
    const res = {
      // should equal 'wvpk'
      BlockID: index.FourCcToken.get(buf, off),
      //  0x402 to 0x410 are valid for decode
      blockSize: index.UINT32_LE.get(buf, off + 4),
      //  0x402 (1026) to 0x410 are valid for decode
      version: index.UINT16_LE.get(buf, off + 8),
      //  40-bit total samples for entire file (if block_index == 0 and a value of -1 indicates an unknown length)
      totalSamples: (
        /* replace with bigint? (Token.UINT8.get(buf, off + 11) << 32) + */
        index.UINT32_LE.get(buf, off + 12)
      ),
      // 40-bit block_index
      blockIndex: (
        /* replace with bigint? (Token.UINT8.get(buf, off + 10) << 32) + */
        index.UINT32_LE.get(buf, off + 16)
      ),
      // 40-bit total samples for entire file (if block_index == 0 and a value of -1 indicates an unknown length)
      blockSamples: index.UINT32_LE.get(buf, off + 20),
      ...
      crc: new index.Uint8ArrayType(4).get(buf, off + 28)
    };
    if (res.flags.isDSD) {
      res.totalSamples *= 8;
    }
...
class WavPackContentError extends index.makeUnexpectedFileContentError("WavPack") {
}
class WavPackParser extends index.BasicParser {
  constructor() {
    super(...arguments);
  ...
  async parseWavPackBlocks() {
    do {
      const blockId = await this.tokenizer.peekToken(index.FourCcToken);
      if (blockId !== "wvpk")
        break;
  ...
  async parseMetadataSubBlock(header, remainingLength) {
    ...
    while (remaining > MetadataIdToken.len) {
      ...
      const dataSizeInWords = await this.tokenizer.readNumber(id.largeBlock ? index.UINT24_LE : index.UINT8);
      ...
      switch (id.functionId) {
        ...
        case 14: {
          debug("ID_DSD_BLOCK");
          const mp = 1 << index.UINT8.get(data, 0);
        ...
        case 38:
          ...
          break;
        case 47:
          debug(`ID_BLOCK_CHECKSUM: checksum=${index.uint8ArrayToHex(data)}`);
          break;
        default:
          debug(`Ignore unsupported meta-sub-block-id functionId=0x${id.functionId.toString(16)}`);
          break;
      }
      remaining -= MetadataIdToken.len + (id.largeBlock ? index.UINT24_LE.len : index.UINT8.len) + dataSizeInWords * 2;

...
const AbstractID3Parser = require("./AbstractID3Parser-BRfoSkZB.js");
const ReplayGain = {
  len: 2,
  get: (buf, off) => {
    const gain_type = index.getBitAllignedNumber(buf, off, 0, 3);
    const sign = index.getBitAllignedNumber(buf, off, 6, 1);
    const gain_adj = index.getBitAllignedNumber(buf, off, 7, 9) / 10;
    if (gain_type > 0) {
      return {
        type: index.getBitAllignedNumber(buf, off, 0, 3),
        origin: index.getBitAllignedNumber(buf, off, 3, 3),
...
const ExtendedLameHeader = {
  len: 27,
  get: (buf, off) => {
    const track_peak = index.UINT32_BE.get(buf, off + 2);
    return {
      revision: index.getBitAllignedNumber(buf, off, 0, 4),
      vbr_method: index.getBitAllignedNumber(buf, off, 4, 4),
      lowpass_filter: 100 * index.UINT8.get(buf, off + 1),
      track_peak: track_peak === 0 ? null : track_peak / 2 ** 23,
      track_gain: ReplayGain.get(buf, 6),
      album_gain: ReplayGain.get(buf, 8),
      music_length: index.UINT32_BE.get(buf, off + 20),
      music_crc: index.UINT8.get(buf, off + 24),
      header_crc: index.UINT16_BE.get(buf, off + 24)
    };
  }
};
const InfoTagHeaderTag = new index.StringType(4, "ascii");
const LameEncoderVersion = new index.StringType(6, "ascii");
const XingHeaderFlags = {
  len: 4,
  get: (buf, off) => {
    return {
      frames: index.isBitSet(buf, off, 31),
      bytes: index.isBitSet(buf, off, 30),
      toc: index.isBitSet(buf, off, 29),
      vbrScale: index.isBitSet(buf, off, 28)
    };
  }
};
async function readXingHeader(tokenizer) {
  ...
  if (flags.frames) {
    xingInfoTag.numFrames = await tokenizer.readToken(index.UINT32_BE);
  }
  if (flags.bytes) {
    xingInfoTag.streamSize = await tokenizer.readToken(index.UINT32_BE);
  }
  if (flags.toc) {
  ...
  if (flags.vbrScale) {
    xingInfoTag.vbrScale = await tokenizer.readToken(index.UINT32_BE);
  }
  const lameTag = await tokenizer.peekToken(new index.StringType(4, "ascii"));
  if (lameTag === "LAME") {
    await tokenizer.ignore(4);
    xingInfoTag.lame = {
      version: await tokenizer.readToken(new index.StringType(5, "ascii"))
    };
...
class MpegContentError extends index.makeUnexpectedFileContentError("MPEG") {
}
...
class MpegFrameHeader {
  constructor(buf, off) {
    ...
    this.frameLength = 0;
    this.versionIndex = index.getBitAllignedNumber(buf, off + 1, 3, 2);
    this.layer = MpegFrameHeader.LayerDescription[index.getBitAllignedNumber(buf, off + 1, 5, 2)];
    if (this.versionIndex > 1 && this.layer === 0) {
      this.parseAdtsHeader(buf, off);
    } else {
      this.parseMpegHeader(buf, off);
    }
    this.isProtectedByCRC = !index.isBitSet(buf, off + 1, 7);
  }
  ...
  parseMpegHeader(buf, off) {
    this.container = "MPEG";
    this.bitrateIndex = index.getBitAllignedNumber(buf, off + 2, 0, 4);
    this.sampRateFreqIndex = index.getBitAllignedNumber(buf, off + 2, 4, 2);
    this.padding = index.isBitSet(buf, off + 2, 6);
    this.privateBit = index.isBitSet(buf, off + 2, 7);
    this.channelModeIndex = index.getBitAllignedNumber(buf, off + 3, 0, 2);
    this.modeExtension = index.getBitAllignedNumber(buf, off + 3, 2, 2);
    this.isCopyrighted = index.isBitSet(buf, off + 3, 4);
    this.isOriginalMedia = index.isBitSet(buf, off + 3, 5);
    this.emphasis = index.getBitAllignedNumber(buf, off + 3, 7, 2);
  ...
  parseAdtsHeader(buf, off) {
    ...
    const profileIndex = index.getBitAllignedNumber(buf, off + 2, 0, 2);
    this.codec = "AAC";
    this.codecProfile = MPEG4.AudioObjectTypes[profileIndex];
    debug(`MPEG-4 audio-codec=${this.codec}`);
    const samplingFrequencyIndex = index.getBitAllignedNumber(buf, off + 2, 2, 4);
    this.samplingRate = MPEG4.SamplingFrequencies[samplingFrequencyIndex];
    debug(`sampling-rate=${this.samplingRate}`);
    const channelIndex = index.getBitAllignedNumber(buf, off + 2, 7, 3);
    this.mp4ChannelConfig = MPEG4_ChannelConfigurations[channelIndex];
    debug(`channel-config=${this.mp4ChannelConfig ? this.mp4ChannelConfig.join("+") : "?"}`);
    this.frameLength = index.getBitAllignedNumber(buf, off + 3, 6, 2) << 11;
  }
  calcBitrate() {
...
class MpegParser extends AbstractID3Parser.AbstractID3Parser {
  ...
  async postId3v2Parse() {
    ...
    try {
      ...
      if (err instanceof index.EndOfStreamError) {
        debug("End-of-stream");
  ...
  async sync() {
    ...
    while (true) {
      ...
      if (this.syncPeek.len <= 163) {
        throw new index.EndOfStreamError();
      }
      while (true) {
  ...
  async parseAdts(header) {
    ...
    header.frameLength += index.getBitAllignedNumber(buf, 0, 0, 11);
  ...
  async parseCrc() {
    await this.tokenizer.ignore(index.INT16_BE.len);
    this.offset += index.INT16_BE.len;
  ...
  async skipSideInformation() {
    if (this.audioFrameHeader) {
      ...
      if (sideinfo_length !== null) {
        await this.tokenizer.readToken(new index.Uint8ArrayType(sideinfo_length));
  ...
  async readXingInfoHeader() {
    ...
    if (infoTag.lame) {
      this.metadata.setFormat("tool", `LAME ${index.stripNulls(infoTag.lame.version)}`);

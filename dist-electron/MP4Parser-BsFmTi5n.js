...
const main = require("./main-C66ZCNkH.js");
const index = require("./index-B01hnH-W.js");
const debug$2 = main.initDebug("music-metadata:parser:MP4:atom");
class Mp4ContentError extends index.makeUnexpectedFileContentError("MP4") {
}
const Header = {
  len: 8,
  get: (buf, off) => {
    const length = index.UINT32_BE.get(buf, off);
    if (length < 0)
      throw new Mp4ContentError("Invalid atom header length");
    return {
      length: BigInt(length),
      name: new index.StringType(4, "latin1").get(buf, off + 4)
    };
  },
  put: (buf, off, hdr) => {
    index.UINT32_BE.put(buf, off, Number(hdr.length));
    return index.FourCcToken.put(buf, off + 4, hdr.name);
  }
};
const ExtendedSize = index.UINT64_BE;
const ftyp = {
  len: 4,
  get: (buf, off) => {
    return {
      type: new index.StringType(4, "ascii").get(buf, off)
    };
  }
};
...
const SecondsSinceMacEpoch = {
  len: 4,
  get: (buf, off) => {
    const secondsSinceUnixEpoch = index.UINT32_BE.get(buf, off) - 2082844800;
...
class MdhdAtom extends FixedLengthAtom {
  ...
  get(buf, off) {
    return {
      version: index.UINT8.get(buf, off + 0),
      flags: index.UINT24_BE.get(buf, off + 1),
      creationTime: SecondsSinceMacEpoch.get(buf, off + 4),
      modificationTime: SecondsSinceMacEpoch.get(buf, off + 8),
      timeScale: index.UINT32_BE.get(buf, off + 12),
      duration: index.UINT32_BE.get(buf, off + 16),
      language: index.UINT16_BE.get(buf, off + 20),
      quality: index.UINT16_BE.get(buf, off + 22)
    };
  }
}
class MvhdAtom extends FixedLengthAtom {
  ...
  get(buf, off) {
    return {
      version: index.UINT8.get(buf, off),
      flags: index.UINT24_BE.get(buf, off + 1),
      creationTime: SecondsSinceMacEpoch.get(buf, off + 4),
      modificationTime: SecondsSinceMacEpoch.get(buf, off + 8),
      timeScale: index.UINT32_BE.get(buf, off + 12),
      duration: index.UINT32_BE.get(buf, off + 16),
      preferredRate: index.UINT32_BE.get(buf, off + 20),
      preferredVolume: index.UINT16_BE.get(buf, off + 24),
      // ignore reserver: 10 bytes
      // ignore matrix structure: 36 bytes
      previewTime: index.UINT32_BE.get(buf, off + 72),
      previewDuration: index.UINT32_BE.get(buf, off + 76),
      posterTime: index.UINT32_BE.get(buf, off + 80),
      selectionTime: index.UINT32_BE.get(buf, off + 84),
      selectionDuration: index.UINT32_BE.get(buf, off + 88),
      currentTime: index.UINT32_BE.get(buf, off + 92),
      nextTrackID: index.UINT32_BE.get(buf, off + 96)
    };
  }
}
class DataAtom {
  ...
  get(buf, off) {
    return {
      type: {
        set: index.UINT8.get(buf, off + 0),
        type: index.UINT24_BE.get(buf, off + 1)
      },
      locale: index.UINT24_BE.get(buf, off + 4),
      value: new index.Uint8ArrayType(this.len - 8).get(buf, off + 8)
    };
  }
}
class NameAtom {
  ...
  get(buf, off) {
    return {
      version: index.UINT8.get(buf, off),
      flags: index.UINT24_BE.get(buf, off + 1),
      name: new index.StringType(this.len - 4, "utf-8").get(buf, off + 4)
    };
  }
}
class TrackHeaderAtom {
  ...
  get(buf, off) {
    return {
      ...
      creationTime: SecondsSinceMacEpoch.get(buf, off + 4),
      modificationTime: SecondsSinceMacEpoch.get(buf, off + 8),
      trackId: index.UINT32_BE.get(buf, off + 12),
      // reserved 4 bytes
      duration: index.UINT32_BE.get(buf, off + 20),
      layer: index.UINT16_BE.get(buf, off + 24),
      alternateGroup: index.UINT16_BE.get(buf, off + 26),
      volume: index.UINT16_BE.get(buf, off + 28)
      // ToDo: fixed point
...
const stsdHeader = {
  ...
  get: (buf, off) => {
    return {
      version: index.UINT8.get(buf, off),
      flags: index.UINT24_BE.get(buf, off + 1),
      numberOfEntries: index.UINT32_BE.get(buf, off + 4)
    };
  }
};
class SampleDescriptionTable {
  ...
  get(buf, off) {
    ...
    return {
      dataFormat: index.FourCcToken.get(buf, off),
      dataReferenceIndex: index.UINT16_BE.get(buf, off + 10),
      description: descrLen > 0 ? new index.Uint8ArrayType(descrLen).get(buf, off + 12) : void 0
    };
  }
}
class StsdAtom {
  ...
  get(buf, off) {
    ...
    for (let n = 0; n < header.numberOfEntries; ++n) {
      const size = index.UINT32_BE.get(buf, off);
      off += index.UINT32_BE.len;
      table.push(new SampleDescriptionTable(size - index.UINT32_BE.len).get(buf, off));
      off += size;
    }
    return {
      header,
...
const SoundSampleDescriptionVersion = {
  ...
  get(buf, off) {
    return {
      version: index.INT16_BE.get(buf, off),
      revision: index.INT16_BE.get(buf, off + 2),
      vendor: index.INT32_BE.get(buf, off + 4)
    };
  }
};
const SoundSampleDescriptionV0 = {
  ...
  get(buf, off) {
    return {
      numAudioChannels: index.INT16_BE.get(buf, off + 0),
      sampleSize: index.INT16_BE.get(buf, off + 2),
      compressionId: index.INT16_BE.get(buf, off + 4),
      packetSize: index.INT16_BE.get(buf, off + 6),
      sampleRate: index.UINT16_BE.get(buf, off + 8) + index.UINT16_BE.get(buf, off + 10) / 1e4
    };
  }
};
class SimpleTableAtom {
  ...
  get(buf, off) {
    const nrOfEntries = index.INT32_BE.get(buf, off + 4);
    return {
      version: index.INT8.get(buf, off + 0),
      flags: index.INT24_BE.get(buf, off + 1),
...
const TimeToSampleToken = {
  ...
  get(buf, off) {
    return {
      count: index.INT32_BE.get(buf, off + 0),
      duration: index.INT32_BE.get(buf, off + 4)
    };
  }
};
...
const SampleToChunkToken = {
  ...
  get(buf, off) {
    return {
      firstChunk: index.INT32_BE.get(buf, off),
      samplesPerChunk: index.INT32_BE.get(buf, off + 4),
      sampleDescriptionId: index.INT32_BE.get(buf, off + 8)
    };
  }
};
...
class StszAtom {
  ...
  get(buf, off) {
    const nrOfEntries = index.INT32_BE.get(buf, off + 8);
    return {
      version: index.INT8.get(buf, off),
      flags: index.INT24_BE.get(buf, off + 1),
      sampleSize: index.INT32_BE.get(buf, off + 4),
      numberOfEntries: nrOfEntries,
      entries: readTokenTable(buf, index.INT32_BE, off + 12, this.len - 12, nrOfEntries)
    };
  }
}
class StcoAtom extends SimpleTableAtom {
  constructor(len) {
    super(len, index.INT32_BE);
    this.len = len;
  }
}
class ChapterText {
  ...
  get(buf, off) {
    const titleLen = index.INT16_BE.get(buf, off + 0);
    const str = new index.StringType(titleLen, "utf-8");
...
class TrackFragmentHeaderBox {
  ...
  get(buf, off) {
    ...
    const header = {
      version: index.INT8.get(buf, off),
      flags: {
        baseDataOffsetPresent: index.getBit(buf, flagOffset + 2, 0),
        sampleDescriptionIndexPresent: index.getBit(buf, flagOffset + 2, 1),
        defaultSampleDurationPresent: index.getBit(buf, flagOffset + 2, 3),
        defaultSampleSizePresent: index.getBit(buf, flagOffset + 2, 4),
        defaultSampleFlagsPresent: index.getBit(buf, flagOffset + 2, 5),
        defaultDurationIsEmpty: index.getBit(buf, flagOffset, 0),
        defaultBaseIsMoof: index.getBit(buf, flagOffset, 1)
      },
      trackId: index.UINT32_BE.get(buf, 4)
    };
    let dynOffset = 8;
    if (header.flags.baseDataOffsetPresent) {
      header.baseDataOffset = index.UINT64_BE.get(buf, dynOffset);
      dynOffset += 8;
    }
    if (header.flags.sampleDescriptionIndexPresent) {
      header.sampleDescriptionIndex = index.UINT32_BE.get(buf, dynOffset);
      dynOffset += 4;
    }
    if (header.flags.defaultSampleDurationPresent) {
      header.defaultSampleDuration = index.UINT32_BE.get(buf, dynOffset);
      dynOffset += 4;
    }
    if (header.flags.defaultSampleSizePresent) {
      header.defaultSampleSize = index.UINT32_BE.get(buf, dynOffset);
      dynOffset += 4;
    }
    if (header.flags.defaultSampleFlagsPresent) {
      header.defaultSampleFlags = index.UINT32_BE.get(buf, dynOffset);
    }
    return header;
  }
}
class TrackRunBox {
  ...
  get(buf, off) {
    ...
    const trun = {
      version: index.INT8.get(buf, off),
      flags: {
        dataOffsetPresent: index.getBit(buf, flagOffset + 2, 0),
        firstSampleFlagsPresent: index.getBit(buf, flagOffset + 2, 2),
        sampleDurationPresent: index.getBit(buf, flagOffset + 1, 0),
        sampleSizePresent: index.getBit(buf, flagOffset + 1, 1),
        sampleFlagsPresent: index.getBit(buf, flagOffset + 1, 2),
        sampleCompositionTimeOffsetsPresent: index.getBit(buf, flagOffset + 1, 3)
      },
      sampleCount: index.UINT32_BE.get(buf, off + 4),
      samples: []
    };
    let dynOffset = off + 8;
    if (trun.flags.dataOffsetPresent) {
      trun.dataOffset = index.UINT32_BE.get(buf, dynOffset);
      dynOffset += 4;
    }
    if (trun.flags.firstSampleFlagsPresent) {
      trun.firstSampleFlags = index.UINT32_BE.get(buf, dynOffset);
      dynOffset += 4;
    }
    for (let n = 0; n < trun.sampleCount; ++n) {
      ...
      if (trun.flags.sampleDurationPresent) {
        sample.sampleDuration = index.UINT32_BE.get(buf, dynOffset);
        dynOffset += 4;
      }
      if (trun.flags.sampleSizePresent) {
        sample.sampleSize = index.UINT32_BE.get(buf, dynOffset);
        dynOffset += 4;
      }
      if (trun.flags.sampleFlagsPresent) {
        sample.sampleFlags = index.UINT32_BE.get(buf, dynOffset);
        dynOffset += 4;
      }
      if (trun.flags.sampleCompositionTimeOffsetsPresent) {
        sample.sampleCompositionTimeOffset = index.UINT32_BE.get(buf, dynOffset);
        dynOffset += 4;
      }
...
class HandlerBox {
  ...
  get(buf, off) {
    const charTypeToken = new index.StringType(4, "utf-8");
    return {
      version: index.INT8.get(buf, off),
      flags: index.UINT24_BE.get(buf, off + 1),
      componentType: charTypeToken.get(buf, off + 4),
      handlerType: charTypeToken.get(buf, off + 8),
      componentName: new index.StringType(this.len - 28, "utf-8").get(buf, off + 28)
    };
  }
}
class ChapterTrackReferenceBox {
  ...
  get(buf, off) {
    ...
    while (dynOffset < this.len) {
      trackIds.push(index.UINT32_BE.get(buf, off + dynOffset));
      dynOffset += 4;
    }
    return trackIds;
  }
...
function distinct(value, index2, self) {
  return self.indexOf(value) === index2;
}
class MP4Parser extends index.BasicParser {
  constructor() {
    super(...arguments);
    ...
    this.atomParsers = {
      ...
      chap: async (len) => {
        ...
        const trackIds = [];
        while (len >= index.UINT32_BE.len) {
          trackIds.push(await this.tokenizer.readNumber(index.UINT32_BE));
          len -= index.UINT32_BE.len;
        }
      ...
      date: async (len) => {
        const date = await this.tokenizer.readToken(new index.StringType(len, "utf-8"));
  ...
  static read_BE_Integer(array, signed) {
    ...
    const token = index.Token[integerType];
    if (!token) {
      ...
      track.soundSampleDescription.forEach((ssd) => {
        ...
        if (ssd.description) {
          ...
          if (description.sampleRate > 0) {
            streamInfo.type = index.TrackType.audio;
            streamInfo.audio = {
  ...
  parseMetadataItemData(metaAtom) {
    ...
    return metaAtom.readAtoms(this.tokenizer, async (child, remaining) => {
      ...
      switch (child.header.name) {
        ...
        default: {
          const uint8Array = await this.tokenizer.readToken(new index.Uint8ArrayType(payLoadLength));
          this.addWarning(`Unsupported meta-item: ${tagKey}[${child.header.name}] => value=${index.uint8ArrayToHex(uint8Array)} ascii=${index.textDecode(uint8Array, "ascii")}`);
        }
      }
    ...
    switch (dataAtom.type.type) {
      case 0:
        switch (tagKey) {
          case "trkn":
          case "disk": {
            const num = index.UINT8.get(dataAtom.value, 3);
            const of = index.UINT8.get(dataAtom.value, 5);
            await this.addTag(tagKey, `${num}/${of}`);
            break;
          }
          case "gnre": {
            const genreInt = index.UINT8.get(dataAtom.value, 1);
            const genreStr = index.Genres[genreInt - 1];
            await this.addTag(tagKey, genreStr);
            break;
          }
          case "rate": {
            const rate = index.textDecode(dataAtom.value, "ascii");
      ...
      case 18:
        await this.addTag(tagKey, index.textDecode(dataAtom.value));
        break;
      case 13:
      ...
      case 22:
        ...
        break;
      case 65:
        await this.addTag(tagKey, index.UINT8.get(dataAtom.value, 0));
        break;
      case 66:
        await this.addTag(tagKey, index.UINT16_BE.get(dataAtom.value, 0));
        break;
      case 67:
        await this.addTag(tagKey, index.UINT32_BE.get(dataAtom.value, 0));
        break;
      default:

...
class WhatsAppManager {
   ...
   */
  getSessionDataPath() {
    return electron.app.isPackaged ? path.join(electron.app.getPath("userData"), ".wwebjs_auth") : ".wwebjs_auth";
  }
  /**
...
class RegionService {
  ...
  async getProvinces() {
    ...
    const res = await this.fetchJson("/provinces.json");
    this.provincesCache = res.data;
    return res.data;
  }
  async getRegencies(provinceCode) {
    const res = await this.fetchJson(`/regencies/${provinceCode}.json`);
    return res.data;
  }
   ...
   */
  async getDistrictsFromKeyword(keyword) {
    try {
...
class MapScraper {
  constructor() {
    ...
    __publicField(this, "isScraping", false);
    __publicField(this, "shouldStop", false);
  }
  async scrapeBingMaps(event, keyword, limit = 50) {
    if (this.isScraping) {
    ...
    const sendProgress = (current, message) => {
  ...
  async processKeyword(page, keyword, results, seenIds, limit, sendProgress, maxNoNewDataRetries = 8) {
    ...
    try {
      ...
      try {
        await page.waitForSelector(".b_vList, .b_lstcards, [data-entity]", { timeout: 1e4 });
      } catch (e) {
      }
      ...
      while (results.length < limit && !this.shouldStop) {
        ...
        const newItems = await page.evaluate(() => {
          ...
          potentialItems.forEach((el) => {
            ...
            if (!entity) {
              const name = (_b = (_a = el.querySelector("h2, .b_factrow strong")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim();
              if (name) {
                const phoneEl = el.querySelector('[data-entity-id*="Phone"], a[href^="tel"]');
                const phone = ((_c = phoneEl == null ? void 0 : phoneEl.textContent) == null ? void 0 : _c.trim()) || "";
                const address = ((_e = (_d = el.querySelector(".address, .b_address")) == null ? void 0 : _d.textContent) == null ? void 0 : _e.trim()) || "";
                const website = ((_f = el.querySelector('a[href^="http"]:not([href*="bing.com"])')) == null ? void 0 : _f.getAttribute("href")) || "";
        ...
        await page.evaluate(() => {
          var _a;
          const candidates = [
            (_a = document.querySelector(".b_vList")) == null ? void 0 : _a.parentElement,
...
const setupIPC = (mainWindow2, wm, mp, qw) => {
  ...
  electron.ipcMain.handle("maps:scrape", async (event, { keyword, limit }) => {
    try {
  ...
  electron.ipcMain.handle("maps:cancel", async () => {

...
class EbmlContentError extends index.makeUnexpectedFileContentError("EBML") {
}
const ParseAction = {
  ReadNext: 0,
...
class EbmlIterator {
  ...
  async parseContainer(dtdElement, posDone, listener) {
    ...
    while (this.tokenizer.position < posDone) {
      ...
      try {
        ...
        if (error instanceof index.EndOfStreamError) {
          break;
        }
        throw error;
      }
  ...
  async readVintData(maxLength) {
    const msb = await this.tokenizer.peekNumber(index.UINT8);
    let mask = 128;
    let oc = 1;
    while ((msb & mask) === 0) {
      if (oc > maxLength) {
  ...
  async readFloat(e) {
    switch (e.len) {
      case 0:
        return 0;
      case 4:
        return this.tokenizer.readNumber(index.Float32_BE);
      case 8:
        return this.tokenizer.readNumber(index.Float64_BE);
      case 10:
        return this.tokenizer.readNumber(index.Float64_BE);
      default:
  ...
  async readString(e) {
    const rawString = await this.tokenizer.readToken(new index.StringType(e.len, "utf-8"));
...
function readUIntBeAsBigInt(buf, len) {
  ...
  try {
    ...
    return index.UINT64_BE.get(normalizedNumber, 0);
  } catch (_error) {
    return BigInt(-1);
  }
}
...
class MatroskaParser extends index.BasicParser {
  constructor() {
    super(...arguments);
        ...
        switch (element.id) {
          ...
          case 524531317:
            if (this.flagUseIndexToSkipClusters && this.seekHead) {
              const index2 = this.seekHead.seek.find((index3) => index3.position + this.seekHeadOffset > this.tokenizer.position);
              if (index2) {
                const ignoreSize = index2.position + this.seekHeadOffset - this.tokenizer.position;
          ...
          case 374648427:
            {
              ...
              if (audioTracks == null ? void 0 : audioTracks.entries) {
                ...
                const audioTrack = audioTracks.entries.filter((entry) => entry.trackType === index.TrackType.audio).reduce((acc, cur) => {
                  if (!acc)
                    return cur;
          ...
          case 307544935:
            {
              ...
              await Promise.all(tags.tag.map(async (tag) => {
                ...
                const targetType = (target == null ? void 0 : target.targetTypeValue) ? index.TargetType[target.targetTypeValue] : (target == null ? void 0 : target.targetType) ? target.targetType : "track";
